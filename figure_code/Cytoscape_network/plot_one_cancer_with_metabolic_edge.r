...
df_to_Cyto <-
  function(df = NULL,
    ...
    typeAll <- setNames(c(
      ...
      rep("gene", length(setdiff(
        unique(njs16_net$gene), unique(df$TF)
      )))
    ),
    c(
      unique(df$TF),
      unique(df$enhancer),
      setdiff(unique(njs16_net$gene), unique(df$TF))
...
for(i in 1:length(name_res)) {
  ...
  # Export image
  #file.remove(paste0(this_name, "_group1_njs16.png"))
  exportImage(
    filename = paste0(this_name, "_group1_njs16_fig.png"),
    type = "png",
    resolution = 300,
    height = 2000,
    width = 2000,
